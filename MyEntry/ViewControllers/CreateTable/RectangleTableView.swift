//
//  RectangleTableView.swift
//  MyEntry
//
//  Created by Vipul Dungranee on 01/03/22.
//

import Foundation
import SwiftUI
//RectangleTableView
struct RectangleTableView: View {
        ...
        GeometryReader { proxy in
            ZStack {
                ForEach(0..<totalSeats) { index in
                    SheetViewForRectangleTable(index: index, totalSeats: totalSeats, sheetSize: sheetSize(proxy.size.width), seat: seats[index], onClickSheet: onClickSheet)
                   
                }
                Color.white
                    .frame(width: circleSize(proxy.size.width), height: circleSize(proxy.size.width / 1.5), alignment: .center)
                    .clipShape(Rectangle())
...
struct RectangleTableView_Previews: PreviewProvider {
    static var previews: some View {
        RectangleTableView(seats: [
...
@available(iOS 14.0, *)
struct SheetViewForRectangleTable: View {
    let index: Int
    var totalSeats: Int
    ...
    func getOffSet() -> CGPoint {
        ...
        var offsetPoint = getOffSet2()
        
        switch getRotationValue() {
        case 0:
            offsetPoint.y += sheetSize / 1.8
        case 90:
            offsetPoint.x += sheetSize / 8.1
        case 180:
            offsetPoint.y -= sheetSize / 1.8
        case 270:
            offsetPoint.x -= sheetSize / 8.1
    ...
    func getOffSet2(multiplier: CGFloat = 1.5) -> CGPoint {
        if totalSeats > 4 {
